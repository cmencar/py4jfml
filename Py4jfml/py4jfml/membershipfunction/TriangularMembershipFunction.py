...
class TriangularMembershipFunction:
    '''
    Python class for representing Triangular functions
    '''
    def __init__(self,p=None,domainLeft=None,domainRight=None):
        '''
        Constructor of class TriangularMembershipFunction
        ...
        if p==None and domainLeft==None and domainRight==None:
            self.java_mf = gateway.entry_point.getJFMLMembershipfunction_Factory().createTriangularMembershipFunction()
        ...
        #Call of the java constructor with Parameter instance with the parameters of the function
            ...
            self.java_mf = gateway.entry_point.getJFMLMembershipfunction_Factory().createTriangularMembershipFunction(p.java_p)

        #Call of the java constructor with Parameter instance with the parameters of the function
        elif p!=None and domainLeft!=None and domainRight!=None:
            assert type(p)==onept.OneParamType or type(p)==twopt.TwoParamType or type(p)==threept.ThreeParamType or type(p)==fourpt.FourParamType
            assert type(domainLeft)==float and type(domainRight)==float
            self.java_mf = gateway.entry_point.getJFMLMembershipfunction_Factory().createTriangularMembershipFunction(p.java_p,domainLeft,domainRight)
