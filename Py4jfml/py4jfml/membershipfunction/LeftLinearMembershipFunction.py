...
class LeftLinearMembershipFunction:
    '''
    Python class for representing LeftLinear membership functions
    '''
    def __init__(self,p=None,domainLeft=None,domainRight=None):
        '''
        Constructor of class LeftLinearMembershipFunction
        :param p: parameter -> a and b
        ...
        if  p==None and domainLeft==None and domainRight==None:
            self.java_mf = gateway.entry_point.getJFMLMembershipfunction_Factory().createLeftLinearMembershipFunction()

        #Call of the java constructor with Parameter instance with parameters of the function
            ...
            self.java_mf = gateway.entry_point.getJFMLMembershipfunction_Factory().createLeftLinearMembershipFunction(p.java_p)

        #Call of the java constructor with Parameter instance with parameters of the function and the left and right domain
        elif p!=None and domainLeft!=None and domainRight!=None:
            assert type(p)==OneParamType or type(p)==TwoParamType or type(p)==ThreeParamType or type(p)==FourParamType
            assert type(domainRight)==float and type(domainLeft)==float
            self.java_mf = gateway.entry_point.getJFMLMembershipfunction_Factory().createLeftLinearMembershipFunction(p.java_p,domainLeft,domainRight)
